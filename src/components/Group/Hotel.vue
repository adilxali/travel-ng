...
const addHotel = () => {
	if (!props.locations[currentLocationIndex.value].hotels) {
		props.locations[currentLocationIndex.value].hotel = {} as any
	}
	props.locations[currentLocationIndex.value].hotel = hotelData.value;
	hotelData.value = {
...
<template>
	<div class="ml-2">
			...
			<n-card>
				...
				<div class="grid grid-cols-3 mt-4 gap-x-2">
					<n-select v-model:value="hotelData.accommodation_id" label-field="title" value-field="id"
						...
						:disabled="locations[currentLocationIndex]?.hotel" />
					<div class="grid grid-cols-2 gap-x-2">
						<n-time-picker v-model:formatted-value="hotelData.check_in_time" format="hh:mm:ss" type="date"
							placeholder="Check in" :disabled="locations[currentLocationIndex]?.hotel" />
						<n-time-picker v-model:formatted-value="hotelData.check_out_time" format="hh:mm:ss" type="date"
							placeholder="Check out" :disabled="locations[currentLocationIndex]?.hotel" />
					</div>
		...
		<div class="mx-auto grid grid-cols-3 gap-x-10 gap-y-2">
			...
			<div v-if="locations[currentLocationIndex].hotel">
				<div class="grid gap-x-4 gap-y-2">
					...
					<div class="bg-white p-4 rounded-lg shadow-md">
						...
						<n-select v-model:value="locations[currentLocationIndex].hotel.accommodation_id" label-field="title" value-field="id"
							:options="currentHotels" placeholder="Select Hotel" />

						<div class="grid grid-cols-2 gap-x-2 mt-2">
							<n-time-picker v-model:formatted-value="locations[currentLocationIndex].hotel.check_in_time" format="hh:mm:ss" type="date"
								placeholder="Check in" />
							<n-time-picker v-model:formatted-value="locations[currentLocationIndex].hotel.check_out_time" format="hh:mm:ss" type="date"
								placeholder="Check out" />
						</div>

						<n-input v-model:value="locations[currentLocationIndex].hotel.room_sharing" placeholder="Room Type" class="mt-2" />
					</div>
				</div>
			</div>
