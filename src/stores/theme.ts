...
export const useThemeStore = defineStore("theme", {
	state: () => ({
		...
		themeName:  ThemeEnum.Light, // Theme name (Dark, Light), with fallback to the light theme
		...
		sidebar: {
			...
			autoCloseBreakpoint: 1800, // Breakpoint for the automatic closing of the sidebar (in pixels)
			animEase: "ease-in-out", // Type of easing for animations
			animDuration: 0.3, // Duration of sidebar animations (in seconds)
			openWidth: 250, // Width of the open sidebar (in pixels)
			closeWidth: 0 // Width of the closed sidebar (in pixels)
		},
		footer: {
		...
		responsive: {
			...
			override: {
				viewPadding: {
					desk: 10, // View padding for desktop
		...
		viewPadding: 10, // Default view padding (in pixels)
	...
	getters: {
		naiveTheme(state): BuiltInGlobalTheme {
			return state.themeName === ThemeEnum.Dark ? lightTheme : lightTheme
		},
